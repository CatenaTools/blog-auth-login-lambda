...
```bash
blog-auth$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
blog-auth$ sam local start-api
blog-auth$ curl http://localhost:3000/
```
...
```bash
blog-auth$ sam logs -n NetCodeWebAPIServerless --stack-name blog-auth --tail
```
...
```bash
blog-auth$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name blog-auth
```
